...
const nodes = [
  { id: 'PRODUCT', label: 'Product\nEngineering', x: 186, y: 92, color: p.lime },
  { id: 'BACKEND', label: 'Backend\nSystems', x: 478, y: 92, color: p.green },
  { id: 'DATA', label: 'Data &\nPipelines', x: 770, y: 92, color: p.teal },
  { id: 'AUTOMATION', label: 'Automation\n& Tooling', x: 1040, y: 92, color: p.cyan },
  { id: 'INTERFACES', label: 'Interfaces\n& Design Systems', x: 186, y: 268, color: p.teal },
  { id: 'INFRA', label: 'Infrastructure\n& DevOps', x: 478, y: 268, color: p.cyan },
  { id: 'SECURITY', label: 'Security\n& Identity', x: 770, y: 268, color: p.green },
  { id: 'ARCH', label: 'Architecture\n& Consulting', x: 1040, y: 268, color: p.lime },
];

function node({ id, label, x, y, color }) {
  const [a, b] = label.split('\n');
  return `
    <g transform="translate(${x} ${y})">
      <rect x="-116" y="-52" width="232" height="104" rx="20" fill="${p.panel}" stroke="${p.line}"/>
      <rect x="-116" y="-52" width="4" height="104" rx="2" fill="${color}"/>
      <circle cx="90" cy="-27" r="5" fill="${color}"/>
...
const lineSvg = connections
  .map(([x1, y1, x2, y2], i) => `<path d="M${x1} ${y1} L${x2} ${y2}" stroke="${i > 7 ? 'url(#accent-soft)' : p.line2}" stroke-width="1.4" stroke-dasharray="${i > 5 ? '5 7' : '0'}" opacity="${i > 7 ? '0.38' : '0.7'}"/>`)
  .join('\n');
...
<svg width="1200" height="360" viewBox="0 0 1200 360" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-labelledby="title desc">
  ...
  <defs>
    <linearGradient id="accent" x1="80" y1="40" x2="1120" y2="320" gradientUnits="userSpaceOnUse">
      <stop stop-color="${p.lime}"/>
      <stop offset="0.36" stop-color="${p.green}"/>
      <stop offset="0.72" stop-color="${p.teal}"/>
      <stop offset="1" stop-color="${p.cyan}"/>
    </linearGradient>
    <linearGradient id="accent-soft" x1="80" y1="40" x2="1120" y2="320" gradientUnits="userSpaceOnUse">
      <stop stop-color="${p.lime}"/>
      <stop offset="0.5" stop-color="${p.teal}"/>
      <stop offset="1" stop-color="${p.cyan}"/>
    </linearGradient>
  </defs>

  <rect width="1200" height="360" rx="28" fill="${p.carbon}"/>
  <rect x="20" y="20" width="1160" height="320" rx="24" fill="${p.graphite}" stroke="${p.line}"/>

  <g opacity="0.78">
    ${lineSvg}
  </g>

  <circle cx="624" cy="180" r="54" fill="${p.panel}" stroke="url(#accent)" stroke-width="2"/>
